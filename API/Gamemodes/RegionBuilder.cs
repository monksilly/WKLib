...
public class RegionBuilder
{
    ...
    public M_Region Build()
    {
        ...
        if (_subregions.Count > 1 && _subregions[0].levelReferences.Count > 1)
        {
            region.transitionLevels =
            [
                new M_Region.TransitionLevels() {
                    fromRegion = _name,
                    levels = [_subregions[0].levelReferences[^1].level]
                }
            ];
            
            _subregions[0].levelReferences.RemoveAt(_subregions[0].levelReferences.Count - 1);
        }
